...
void saySomething(int nWho)
{
	...
	if (nbSpeak[nWho]==0)
	{
		...
		else
		{
			...
			if (nWho==P_MALE)
			{
				...
				fprintf(f,"Hello hunk you are looking as handsome as ever.\n");
				fprintf(f,"How is your day going?\n");							
			}
			if (nWho==P_FEMALE)
			{
	...
	if (nbSpeak[nWho] % 5==0)
	{
		...
		else
		{
			...
			if (nWho==P_FEMALE) system("./femaleAds.sh");
			if (nWho==P_MALE) system("./maleAds.sh");
			if (nWho==P_UNKNOWN1) system("omxplayer -o hdmi minecraft.mp4");
...
int main(int argc, const char *argv[]) {
	...
	// life is hard.
	people[P_FEMALE] 	= "Female";
	people[P_MALE] 		= "Male";
	people[P_UNKNOWN1] 	= "Unknown1";
	people[P_UNKNOWN2]	= "Unknown2";
